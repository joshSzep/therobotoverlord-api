...
async def websocket_endpoint(
    ...
    try:
        ...
        websocket_manager.subscribe_to_channel(
            connection_id, f"user_{user.pk}_notifications"
        )
        ...
        await event_broadcaster.broadcast_user_activity_update(
            user_id=user.pk, username=user.username, status="online"
        )
        ...
        if user:
            ...
            try:
                event_broadcaster = get_event_broadcaster(websocket_manager)
                await event_broadcaster.broadcast_user_activity_update(
                    user_id=user.pk, username=user.username, status="offline"
                )
            except Exception as e:
                logger.error(f"Failed to broadcast user offline status: {e}")
...
async def handle_chat_message(
    ...
    await event_broadcaster.broadcast_overlord_chat_message(
        ...
        metadata={"user_message": chat_text, "response_type": "placeholder"},
    )
