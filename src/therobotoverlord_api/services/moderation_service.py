...
class ModerationService:
    ...
    async def process_content_decision(
        ...
        try:
            ...
            logger.info(f"Processed {decision} decision for {content_type} {content_id}")

        except Exception as e:
            logger.error(f"Error processing moderation decision: {e}")
            raise

    async def _update_post_status(self, post_id: UUID, decision: str, feedback: str | None):
    ...
    async def _update_topic_status(self, topic_id: UUID, decision: str, feedback: str | None):
        ...
        query = """
            ...
            WHERE pk = $3
        """
        await self.db.execute(query, status, feedback, topic_id)
    ...
    async def _update_queue_positions(self, content_type: str, user_id: UUID):
