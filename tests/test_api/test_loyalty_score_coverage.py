...
class TestLoyaltyScoreAPICoverage:
    ...
    async def test_update_loyalty_score_unauthorized(self, async_client: AsyncClient):
        ...
        response = await async_client.post("/api/v1/loyalty-score/update", json=update_data)
        assert response.status_code in [status.HTTP_401_UNAUTHORIZED, status.HTTP_403_FORBIDDEN]
    ...
    @pytest.mark.asyncio
    async def test_get_loyalty_predictions_unauthorized(self, async_client: AsyncClient):
    ...
    async def test_bulk_update_loyalty_unauthorized(self, async_client: AsyncClient):
        ...
        response = await async_client.post("/api/v1/loyalty-score/bulk-update", json=bulk_data)
        assert response.status_code in [status.HTTP_401_UNAUTHORIZED, status.HTTP_403_FORBIDDEN]
    ...
    async def test_loyalty_score_with_invalid_data(self, async_client: AsyncClient):
        ...
        response = await async_client.post("/api/v1/loyalty-score/update", json=invalid_data)
