...
class TestPostRepository:
    ...
    @pytest.fixture
    async def post_repository(self):
    ...
    async def test_create_post(
        self, post_repository, sample_post_create, mock_db_connection
    ):
    ...
    async def test_create_reply_post(
        self, post_repository, reply_post_create, mock_db_connection
    ):
    ...
    async def test_update_post(self, post_repository, mock_db_connection):
    ...
    async def test_get_by_topic(self, post_repository, mock_db_connection):
    ...
    async def test_get_by_topic_with_status_filter(
        self, post_repository, mock_db_connection
    ):
        ...
        result = await post_repository.get_by_topic(
            topic_pk, status=ContentStatus.APPROVED, limit=10, offset=0
        )

        assert len(result) == 1
        assert result[0].status == ContentStatus.APPROVED

    async def test_get_by_author(self, post_repository, mock_db_connection):
    ...
    async def test_get_thread_view(self, post_repository, mock_db_connection):
        ...
        assert result[1].thread_depth == 1

    async def test_approve_post(self, post_repository, mock_db_connection):
        ...
        result = await post_repository.approve_post(
            post_pk, feedback="Well reasoned, Citizen."
        )

        assert result.status == ContentStatus.APPROVED
        assert result.overlord_feedback == "Well reasoned, Citizen."
        assert result.approved_at == approved_at

    async def test_reject_post(self, post_repository, mock_db_connection):
        ...
        mock_record = {
            ...
            "overlord_feedback": "This violates our standards.",
            "submitted_at": datetime.now(UTC),
            "approved_at": None,
        }
        ...
        result = await post_repository.reject_post(
            post_pk, feedback="This violates our standards."
        )

        assert result.status == ContentStatus.REJECTED
        assert result.overlord_feedback == "This violates our standards."
        assert result.approved_at is None

    async def test_search_posts(self, post_repository, mock_db_connection):
        ...
        mock_records = [
            {
                ...
                "parent_post_pk": None,
                "author_pk": uuid4(),
                "content": "This post contains the search term",
                "status": ContentStatus.APPROVED.value,
                "overlord_feedback": None,
                "submitted_at": datetime.now(UTC),
                "approved_at": datetime.now(UTC),
        ...
        result = await post_repository.search_posts("search", limit=10, offset=0)

        assert len(result) == 1
        assert isinstance(result[0], PostWithAuthor)
        assert "search" in result[0].content.lower()
    ...
    async def test_search_posts_with_topic_filter(
        ...
        result = await post_repository.search_posts(
            "search", topic_pk=topic_pk, limit=10, offset=0
        )

        assert len(result) == 1
        assert result[0].topic_pk == topic_pk

    async def test_get_graveyard_posts(self, post_repository, mock_db_connection):
        """Test getting graveyard (rejected) posts."""
        mock_records = [
            {
                "pk": uuid4(),
                "topic_pk": uuid4(),
                "topic_title": "Graveyard Topic",
    ...
    async def test_get_nonexistent_post(self, post_repository, mock_db_connection):
        """Test getting a non-existent post."""
        mock_db_connection.fetchrow.return_value = None

        result = await post_repository.get(uuid4())

        assert result is None

    async def test_update_nonexistent_post(self, post_repository, mock_db_connection):
    ...
    async def test_delete_post(self, post_repository, mock_db_connection):
        """Test deleting a post."""
        post_pk = uuid4()
        mock_db_connection.execute.return_value = "DELETE 1"

        result = await post_repository.delete(post_pk)

        assert result is True

    async def test_delete_nonexistent_post(self, post_repository, mock_db_connection):
