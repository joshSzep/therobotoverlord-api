...
class TestTopicCreationQueueRepository:
    ...
    async def test_create_queue_item(
        self, topic_queue_repository, sample_topic_queue_create, mock_connection
    ):
        ...
        with patch(
            ...
            mock_record = {
                ...
                "updated_at": None,
                "content_pk": sample_topic_queue_create.content_pk,
                "priority_score": sample_topic_queue_create.priority_score,
                "priority": sample_topic_queue_create.priority,
                "position_in_queue": sample_topic_queue_create.position_in_queue,
                "entered_queue_at": sample_topic_queue_create.entered_queue_at,
            ...
            result = await topic_queue_repository.create(sample_topic_queue_create)

        assert isinstance(result, TopicCreationQueue)
        assert result.content_pk == sample_topic_queue_create.content_pk
        assert result.priority_score == sample_topic_queue_create.priority_score
        assert result.status == QueueStatus.PENDING

    async def test_get_queue_position(self, topic_queue_repository, mock_connection):
        ...
        with patch(
            ...
            result = await topic_queue_repository.get_queue_position(content_pk)

        assert result == 3

    async def test_get_queue_position_not_found(
        self, topic_queue_repository, mock_connection
    ):
    ...
    async def test_update_queue_positions(
        self, topic_queue_repository, mock_connection
    ):
        ...
        with patch(
            ...
            result = await topic_queue_repository.update_queue_positions()
