...
class TestBadgeService:
    ...
    async def test_create_badge(self, mock_badge_repo, service, mock_badge):
        ...
        mock_repo_instance.create_from_dict.assert_called_once_with(badge_data.model_dump())
        assert result == mock_badge
    ...
    async def test_update_badge_with_data(self, mock_badge_repo, service, mock_badge):
        ...
        badge_id = uuid4()
        badge_data = BadgeUpdate(name="Updated Badge", description="Updated description")
        mock_repo_instance = AsyncMock()
    ...
    async def test_delete_badge_success(self, mock_badge_repo, service):
        ...
        mock_repo_instance.update_from_dict.return_value = Badge(
            pk=badge_id, name="Test", description="Test", image_url="test.png",
            badge_type=BadgeType.POSITIVE, criteria_config={}, is_active=False,
        ...
        mock_repo_instance.update_from_dict.assert_called_once_with(badge_id, {"is_active": False})
    ...
    async def test_get_user_badges(self, mock_user_badge_repo, service):
        ...
        user_id = uuid4()
        mock_badge = Badge(
            pk=uuid4(), name="Test Badge", description="Test", image_url="test.png",
            badge_type=BadgeType.POSITIVE, criteria_config={}, is_active=True,
            created_at=datetime.now(UTC), updated_at=datetime.now(UTC)
        )
        mock_badges = [UserBadgeWithDetails(
            pk=uuid4(), user_pk=user_id, badge_pk=uuid4(),
            awarded_at=datetime.now(UTC), awarded_by_event="test",
            created_at=datetime.now(UTC), updated_at=datetime.now(UTC),
            badge=mock_badge
        )]
        mock_repo_instance = AsyncMock()
    ...
    @patch("therobotoverlord_api.services.badge_service.UserBadgeRepository")
    @patch("therobotoverlord_api.services.badge_service.BadgeRepository")
    async def test_manually_award_badge_success(self, mock_badge_repo, mock_user_badge_repo, service, mock_badge, mock_user_badge):
        ...
        result = await service.manually_award_badge(user_id, badge_id, awarded_by_user_id)

        mock_user_badge_repo_instance.has_badge.assert_called_once_with(user_id, badge_id)
    ...
    @pytest.mark.asyncio
    @patch("therobotoverlord_api.services.badge_service.UserBadgeRepository")
    async def test_manually_award_badge_already_has(self, mock_user_badge_repo, service):
    ...
    @patch("therobotoverlord_api.services.badge_service.UserBadgeRepository")
    @patch("therobotoverlord_api.services.badge_service.BadgeRepository")
    async def test_manually_award_badge_inactive(self, mock_badge_repo, mock_user_badge_repo, service):
        ...
        inactive_badge = Badge(
            pk=badge_id, name="Inactive", description="Inactive badge",
            image_url="inactive.png", badge_type=BadgeType.POSITIVE,
            criteria_config={}, is_active=False,
            created_at=datetime.now(UTC), updated_at=datetime.now(UTC)
        )
    ...
    @pytest.mark.asyncio
    @patch("therobotoverlord_api.services.badge_service.UserBadgeRepository")
    async def test_revoke_badge_success(self, mock_user_badge_repo, service, mock_user_badge):
        ...
        mock_repo_instance.find_one_by.assert_called_once_with(user_pk=user_id, badge_pk=badge_id)
    ...
    @patch("therobotoverlord_api.services.badge_service.UserBadgeRepository")
    @patch("therobotoverlord_api.services.badge_service.BadgeRepository")
    async def test_evaluate_badge_criteria_for_user(self, mock_badge_repo, mock_user_badge_repo, service, mock_badge):
        ...
        with patch.object(service, '_check_badge_eligibility') as mock_check:
            mock_eligibility = BadgeEligibilityCheck(
                badge_pk=mock_badge.pk, badge_name=mock_badge.name,
                is_eligible=True, current_progress=5, required_progress=5,
                criteria_met=True, reason="Criteria met"
            )
    ...
    async def test_check_approved_posts_criteria(self, mock_get_connection, service, mock_badge):
        ...
        result = await service._check_approved_posts_criteria(user_id, mock_badge, criteria)
    ...
    @pytest.mark.asyncio
    @patch("therobotoverlord_api.services.badge_service.get_db_connection")
    async def test_check_rejected_posts_criteria(self, mock_get_connection, service, mock_badge):
        """Test checking rejected posts criteria."""
        user_id = uuid4()
        criteria = {"type": "rejected_posts", "count": 3, "criteria": "strawman_fallacy"}
        mock_badge.criteria_config = criteria
        ...
        result = await service._check_rejected_posts_criteria(user_id, mock_badge, criteria)
    ...
    @pytest.mark.asyncio
    @patch("therobotoverlord_api.services.badge_service.get_db_connection")
    async def test_check_successful_appeals_criteria(self, mock_get_connection, service, mock_badge):
        ...
        result = await service._check_successful_appeals_criteria(user_id, mock_badge, criteria)
    ...
    @pytest.mark.asyncio
    @patch("therobotoverlord_api.services.badge_service.get_db_connection")
    async def test_check_first_approved_post_criteria(self, mock_get_connection, service, mock_badge):
    ...
    @pytest.mark.asyncio
    @patch("therobotoverlord_api.services.badge_service.UserBadgeRepository")
    async def test_auto_award_eligible_badges(self, mock_user_badge_repo, service, mock_user_badge):
        """Test automatically awarding eligible badges."""
        user_id = uuid4()
        event_type = "post_approved"
        
        mock_eligibility = BadgeEligibilityCheck(
            badge_pk=uuid4(), badge_name="Test Badge",
            is_eligible=True, current_progress=5, required_progress=5,
            criteria_met=True, reason="Criteria met"
        )
    ...
    @pytest.mark.asyncio
    @patch("therobotoverlord_api.services.badge_service.UserBadgeRepository")
    async def test_auto_award_eligible_badges_exception(self, mock_user_badge_repo, service):
    ...
    async def test_get_badge_recipients(self, mock_user_badge_repo, service):
        ...
        limit = 50
        mock_badge = Badge(
            pk=badge_id, name="Test Badge", description="Test", image_url="test.png",
            badge_type=BadgeType.POSITIVE, criteria_config={}, is_active=True,
            created_at=datetime.now(UTC), updated_at=datetime.now(UTC)
        )
        mock_recipients = [UserBadgeWithDetails(
            pk=uuid4(), user_pk=uuid4(), badge_pk=badge_id,
            awarded_at=datetime.now(UTC), awarded_by_event="test",
            created_at=datetime.now(UTC), updated_at=datetime.now(UTC),
            badge=mock_badge
        )]
